 ...
 * @author Rod Byrne
 */
public class BPReport extends HttpServlet {
    ...
    throws ServletException, IOException
    {
        String[] users;
        try {
            users = bpm.getUsers(); //update the users
        }
        ...
        for( String u : users ) {
            ...
            out.printf("<label><input type='checkbox' %s name='users' value='%s'>%s</label> ",checked,u,u);
        }
        out.println("<br>");
        out.println("Dates:");
        String sd =  dtFormat.format( select.startDate );
        out.printf("<label>Start <input name='start_date' type='text' size='15' value='%s'></label>",sd);
        String ed =  dtFormat.format( select.endDate );
        out.printf("<label>End <input name='end_date' type='text' size='15' value='%s'></label><br>",ed);
        out.println("Systolic:");
        out.printf("<label>Low <input name='low_sys' type='text' size='5' value='%.1f'></label>", select.lowSys);
        out.printf("<label>High <input name='hi_sys' type='text' size='5' value='%.1f'></label><br>", select.hiSys);
        out.println("Diastolic:");
        out.printf("<label>Low <input name='low_dia' type='text' size='5' value='%.1f'></label>",select.lowDia);
        out.printf("<label>High <input name='hi_dia' type='text' size='5' value='%.1f'></label><br>",select.hiDia);
        out.println("Pulse Rate:");
        out.printf("<label>Low <input name='low_pulse' type='text' size='5' value='%.1f'></label>",select.lowPulse);
        out.printf("<label>High <input name='hi_pulse' type='text' size='5' value='%.1f'></label><br>",select.hiPulse);
        ...
        try {
            ...
            out.println("</tr>");
            List<BloodPressure> records = bpm.getRecords( select );
            double sumSystolic = 0.0, sumDiastolic = 0.0, sumPulse = 0.0;
            for( BloodPressure rec : records ) {
                out.println("<tr>");
    ...
    private ReportSelect mkReportSelect( HttpServletRequest request ) {
        try {
            ...
            select.startDate = dtFormat.parse(request.getParameter("start_date"));
            ...
            select.lowPulse = Double.parseDouble(request.getParameter("low_pulse"));
            select.hiPulse = Double.parseDouble(request.getParameter("hi_pulse"));
            return select;
        }
        catch( Exception ex ) {
            return null;
    ...
    public void init(ServletConfig config) throws ServletException {
        ...
        try {
            ...
            defaultSelect.users = bpm.getUsers();
        }
        catch( BPRMException ex ) {
            bpm = null;
            log( ex.getMessage() );
