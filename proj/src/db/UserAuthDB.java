...
public class UserAuthDB {
	...
	public UserAuthDB(String path) throws SQLException {
		...
		try {
			...
			stmt.executeUpdate("create table if not exists roles ("
					+ "id integer primary key," + "role text not null unique);");
	...
	public List<String> getUserNames() throws SQLException {
		...
		try {
			conn = openConnection();
			stmt = conn
					.prepareStatement("select username from users order by username;");
	...
	public List<String> getRoleNames() throws SQLException {
		...
		try {
			stmt = conn
					.prepareStatement("select role from roles order by role;");
			ResultSet rs = stmt.executeQuery();
			while (rs.next()) {
				list.add(rs.getString(1));
			}
	...
	public long getRoleId(String role) throws SQLException {
		...
		try {
			conn = openConnection();
			stmt = conn.prepareStatement("select id from roles where role = ?");
	...
	public long addUser(String user, String pw) throws SQLException {
		...
		try {
			...
			PreparedStatement maxid = conn
					.prepareStatement("select last_insert_rowid() from users;");
	...
	public boolean addUserRole(String user, String role) throws SQLException {
		...
		try {
			...
			PreparedStatement count = conn
					.prepareStatement("select count(*) from user_roles where "
	...
	public List<String> getUserRoles(String user) throws SQLException {
		...
		try {
			conn = openConnection();
			stmt = conn
					.prepareStatement("select roles.role from user_roles, roles "
	...
	public void deleteUserRole(String user, String role) throws SQLException {
		...
		try {
			conn = openConnection();
			stmt = conn.prepareStatement("delete from user_roles where "
	...
	public void deleteUser(String user) throws SQLException {
		...
		try {
			conn = openConnection();
			PreparedStatement stmt = conn
					.prepareStatement("delete from user_roles where "
							+ "user_id=(select id from users where username = ?);");
			stmt.setString(1, user);
			stmt.executeUpdate();
			stmt.close();
